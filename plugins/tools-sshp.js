...
var handler = async (m, { conn, command, args }) => {
  ...
  try {
    var img = await fetch(`https://api.betabotz.org/api/tools/ssweb?device=phone&url=${url}&apikey=${lann}`);
    if (!img) {
      await m.reply('Gagal saat percobaan pertama. Memulai percobaan kedua...');
      img = await fetch(`https://api.betabotz.org/api/tools/ssweb?device=phone&url=${url}&apikey=${lann}`);
