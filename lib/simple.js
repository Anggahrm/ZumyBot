...
`.trim()
     ...
     * @param {Object} options
     */
    conn.zumy = async (jid, text = '', quoted, options) => {
    ...
    conn.zumz = async (jid, text = '', quoted, options) => {
  const loadingMsg = "Loading...";

  const { key } = await conn.sendMessage(jid, { text: `${loadingMsg} 0%` });

  for (let i = 10; i <= 100; i += 10) {
    await new Promise(resolve => setTimeout(resolve, 1000));
    await conn.sendMessage(jid, { text: `${loadingMsg} ${i}%`, edit: key });
  }
  ...
  return Buffer.isBuffer(text)
    ...
    conn.zums = async (jid, text = '', quoted, options) => {
  const loadingMsg = "Loading...";
  const successMsg = "Successfully!";

const { key } = await conn.sendMessage(jid, { text: `${loadingMsg}` });
  ...
  await new Promise(resolve => setTimeout(resolve, 1000));
  await conn.sendMessage(jid, { text: `${loadingMsg} 100%`, edit: key });

  await new Promise(resolve => setTimeout(resolve, 1000));
  await conn.sendMessage(jid, { text: `${text}`, edit: key });
  ...
  return Buffer.isBuffer(text)
    ...
    conn.angga = async (jid, text = '', quoted, options) => {
  const arr = [
    "Loading... 0%",
    "Loading... 10%",
    "Loading... 30%",
  ...
  const { key } = await conn.sendMessage(jid, { text: 'Please Wait...' });

  for (let i = 0; i < arr.length; i++) {
    await new Promise(resolve => setTimeout(resolve, 1000));
    await conn.sendMessage(jid, { text: arr[i], edit: key });
  }
  return Buffer.isBuffer(text)
    ? conn.sendFile(jid, text, 'file', '', quoted, false, options)
    : conn.sendMessage(jid, {
